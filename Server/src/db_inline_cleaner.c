...
{
    int success, negate, f, delcnt, aflags, i_array[10000], 
        i_loop, i_loopcur, anum;
    char *buff, *sp, *p, *q, *lbuff, *s_chkattr, *cp, *cpbuf, *cpptr;
    dbref i, aowner;
    VATTR *va;
    ATTR *va2;
    ...
    *p = '\0';
    ...
    switch (key) {
    case ATTRIB_CLEAN:
       i_loop = mudstate.attr_next / 310000;
       cpptr = cpbuf = alloc_lbuf("attrib linking");
       notify(player, safe_tprintf(cp, &cpptr, "Marking attributes.  %d total passes.", i_loop));
       for ( i_loopcur = 0; i_loopcur <= i_loop; i_loopcur++ ) {
           DO_WHOLE_DB(i) {
              for (anum = atr_head(i, &cp); anum; anum = atr_next(&cp)) {
                 if ( (anum > (1 + (310000 * i_loopcur))) && (anum < (310000 * (1 + i_loopcur))) )
                    i_array[anum % 10000] |= 1 << ((anum / 10000) % 32);
              }
           }
           cpptr = cpbuf;
           notify(player, safe_tprintf(cpbuf, &cpptr, "Pass #%d/%d done.", i_loopcur, i_loop));
       }
       notify(player, "Attributes marked.");
       for (i = 0; i < 10000; i++ ) {
          cpptr = cpbuf;
          notify(player, safe_tprintf(cpbuf, &cpptr, "Value: %d - %08x", i, i_array[i]));
       }
       free_lbuf(cpbuf);
       break;
    ...
    if ( (key != ATTRIB_DELETE) && (key != ATTRIB_CLEAN) ) {
